...
#' Title make_manhattan_zoom_annotation
#'
#' @param out.dir where to output plot
#' @param plot.type png or pdf output?
#' @param trait.name character string of trait being plot
#' @param top.hits table of top hits with column names "CHROM" and "POS"
#' @param window window in KB around top snp to plot
#' @param geno.bed name of bed file, no trailing .bed
#' @param plot.r2.thresh R2 threshold for plotting hits
#' @param file.prefix optional prefix for talking about
#' @param gwas.res table of all gwas hits, what is plotted in manhattan around top hits
#'
#' @return output a manhattan plot with genes and ld to outdir
#' @export
#'
#' @examples
make_manhattan_zoom_annotation <- function(out.dir = "./",
  ...
  if(nrow(top.hits) > 0){
    ...
    for(i in 1:nrow(top.hits)){
      ...
      this.fcpu.hit <- filter(top.hits, SNP == this.snp.name)

      make.ld(this.snp.name, window, geno.bed, "/scratch/inputs/")
      ...
      gwas.sub <- gwas.res %>%
        filter(CHROM == this.chr) %>%
        filter(between(POS, this.pos - window * 1000, this.pos + window * 1000)) %>%
      ...
      man <-
        ggplot(aes(x = POS, y = logpval), data = plot.df) +
        geom_point(aes(color = plot.R2, alpha = how.to.plot), shape = 16) +
        scale_alpha(guide = "none") +
        geom_point(aes(x = POS, y = -log10(.data[[paste0(trait.name, ".FarmCPU")]])), data = this.fcpu.hit, fill = "red", size = 4, shape = 23) +
      ...
      ldsmooth <-
        ggplot(aes(x = POS, y = R2), data = gwas.sub) +
      ...
      anno.sub <- anno.full %>%
        ...
        mutate(plot_name = case_when(is.na(GeneNameShort) ~ Description,
                                     TRUE ~ paste0(GeneNameShort, ", ", Description))) %>%
        filter(CHROM == this.chr) %>%
        rowwise() %>%
        mutate(dist.from.snp = get.gene.dist.from.snp(this.pos, start, end)) %>%
        filter(dist.from.snp <= window * 1000)
      ...
      anno <-
        ...
        geom_segment(aes(y = start, yend = end, x = .5, xend = .5), linewidth = 2, color = "red") +
        ...
        #           hjust = 0,
        #           size = 2) +
        geom_fit_text(aes(xmin = .55, xmax = .85, y = y.pos, label = paste0(plot_name)),
                      place = "left",
                      #grow = TRUE,
                      hjust = 0,
                      padding.y = grid::unit(.1, "lines"),
                      min.size = text.min.size) +
        geom_segment(aes(x = .5, xend = .55, y = start, yend = y.pos)) +
        theme(axis.text.x = element_blank(),
      ...
      out.plot <- man + ldsmooth + anno +
        plot_layout(nrow = 1, widths = c(3,1,6)) +
        plot_annotation(title = paste(this.snp.name, trait.name, sep = ", "))
